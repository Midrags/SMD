...
from smd.storage.vdf import ensure_library_has_app
...
class UI:
    ...
    def steam_patch_menu(self) -> MainReturnCode:
        """Patch or unpatch Steam with DLLs so client reads stplug-in."""
        status = get_patch_status(self.steam_path)
        status_msg = {
            "patched": Fore.GREEN + "Steam is patched (DLLs installed)." + Style.RESET_ALL,
            "not_patched": Fore.YELLOW + "Steam is not patched." + Style.RESET_ALL,
            "partial": Fore.YELLOW + "Steam is partially patched (one DLL missing)." + Style.RESET_ALL,
        }[status]
        print(Fore.CYAN + "\n=== Patch / Unpatch Steam (DLL) ===" + Style.RESET_ALL)
        print(status_msg)
        ...
        if not dll_dir:
            print(
                Fore.RED
                + "DLLs not found. Place xinput1_4.dll and hid.dll in the steam_patch folder."
                + Style.RESET_ALL
            )
            return MainReturnCode.LOOP
        ...
        choices = []
        if status != "patched":
            choices.append(("Patch Steam (install DLLs)", "patch"))
        if status in ("patched", "partial"):
            choices.append(("Unpatch Steam (remove DLLs)", "unpatch"))
        choices.append(("Back", "back"))
        key = prompt_select("Choose:", choices, cancellable=True)
        if not key or key == "back":
            return MainReturnCode.LOOP
        if key == "patch":
            ok, msg = patch_steam(self.steam_path)
            print(Fore.GREEN + msg + Style.RESET_ALL if ok else Fore.RED + msg + Style.RESET_ALL)
        else:
            ok, msg = unpatch_steam(self.steam_path)
            print(Fore.GREEN + msg + Style.RESET_ALL if ok else Fore.RED + msg + Style.RESET_ALL)
        return MainReturnCode.LOOP
